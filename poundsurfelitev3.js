...
(async function() {
    ...
    function sleep(min, max){
        const milisecons = getRandomMillisecond(min,max);
        console.log(milisecons);
        return new Promise(resolve => setTimeout(resolve, milisecons));
    }
    ...
    function calculatePetValue(pet)
    {
        ...
        if((blacklist.has(pet.name) || blacklistSpecies.has(pet.species) || blacklistColors.has(pet.color)) && totalValue < 20)
        {
            totalValue = 0;
        }

        if(regexEnabled && totalValue < 30)
        {
    ...
    if(refreshEnabled)
    {
        if(petFound != true)
        {
            setTimeout(function(){
                location.reload();
            }, Math.floor(1100 + Math.random() * 300));
        }
    }
})();